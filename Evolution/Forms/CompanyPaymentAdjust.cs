...
namespace Evolution.Forms
{
    public partial class CompanyPaymentAdjust : Form
    {
        ...
        {
            DVPayment = SQLCMD.SQLdata("LS_CompanyPaymentAdjust_L "+AgreementID+","+ CompanyPercentID + "").DefaultView;
          ...
          //  if (Amount.Text == "" || decimal.Parse(Amount.Text)<=0) { MessageBox.Show("Missing Amount", "OWNER", MessageBoxButtons.OK, MessageBoxIcon.Warning); Amount.Focus(); return; }
           ...
           // CreationDate = ApplicationDate.Text;
            ...
            try
            {
                ...
                /*---------------------------------------------------------------------------------------------------*/
                DVSave = SQLCMD.SQLdata("LS_CompanyPaymentAdjust_M " + ((CompanyPaymentAdjustID == "0") ? "0" : "1") + "," + ((DP<=0)? "0" : CompanyPaymentAdjustID ) + "," + AgreementID + ","+
                    CompanyPercentID + "," + DP + "," +TransactionTypeList.SelectedValue + ",1," + General.Globalvariables.guserid + ",'"+ApplicationDate.Text+"'").DefaultView;
                /*---------------------------------------------------------------------------------------------------*/
                DVSave = SQLCMD.SQLdata("LS_CompanyPaymentAdjust_M " + ((CompanyPaymentAdjustID == "0") ? "0" : "1") + "," + ((MR <= 0) ? "0" : CompanyPaymentAdjustID) + "," + AgreementID + "," +
        ...
        private void Btndelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DVSave = SQLCMD.SQLdata("LS_CompanyPaymentAdjust_M 2," + CompanyPaymentAdjustID + "," + AgreementID + ",0,0,0,0," + General.Globalvariables.guserid + ",'01-01-1990'").DefaultView;
                FillGrid();
                Btnnew.PerformClick();
                MessageBox.Show("Done", "OWNER", MessageBoxButtons.OK, MessageBoxIcon.Information);
            } catch(Exception ecx) { MessageBox.Show(ecx.Message, "OWNER", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        ...
        private void PaymentAdjustList_CellDoubleClick(object sender, Telerik.WinControls.UI.GridViewCellEventArgs e)
        {
            ...
            CompanyPaymentAdjustID = PaymentAdjustList.CurrentRow.Cells["CompanyPaymentAdjustID"].Value.ToString();
